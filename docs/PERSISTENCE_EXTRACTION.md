...
Scan lifecycle persistence was extracted to **[cafe-persistence](https://github.com/create2-labs/cafe-persistence)** in **PERS-D1**. Stack deploy switched to `oleglod/cafe-persistence` in **PERS-D2** (`cafe-deploy` compose service `cafe-discovery-persistence`, unchanged name — option B).
...
| Compose service name | `cafe-discovery-persistence` (see `cafe-deploy/compose/20-discovery.yml`) |
